...
/**
 * 
 * 解决 Mybatis 中枚举的问题， 获取 ResultSet 的值都是获取字符串的，然后比较字符串，以便通用。
 *
 * @author
 * @version 1.0
 * @since
...
public class BaseEnumTypeHandler<E extends Enum<E> & BaseEnum<?, ?>> extends BaseTypeHandler<E> {
	...
	public void setNonNullParameter(PreparedStatement ps, int i, E parameter, JdbcType jdbcType) throws SQLException {
		/*
		 * BaseTypeHandler已经帮我们做了parameter的null判断 数据库存储的是枚举的值，所以我们这里使用 value ， 如果需要存储
		 * name，可以自定义修改
		 */
		if (jdbcType == null) {
	...
	private E locateEnumStatus(String value) {
		for (E e : enums) {
			if (Objects.toString(e.getValue()).equalsIgnoreCase(value)) {
				return e;
			}
		}
